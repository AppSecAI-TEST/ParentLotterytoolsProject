...
{
	var url = contextPath + '/lotteryManage/getDetailLotteryPlay.action';
...
{
	var url = contextPath + '/lotteryManage/deleteLotteryPlays.action';
	var data1 = new Object();
...
function getCheckIpbuId(lpId)
{
	...
	$.ajax({
		async: false,   //设置为同步获取数据形式
        type: "get",
        url: contextPath+'/lotteryManage/getLotteryPlayPlanId.action',
        data:data,
        dataType: "json",
        success: function (data) {
...
{
	$('#ff').form('submit',{
		url:contextPath+'/lotteryManage/saveOrUpdateLotteryPlay.action',
