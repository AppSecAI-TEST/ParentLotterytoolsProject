...
function deleteImgsByNewsuuid(newsUuid)
{
	...
	$.ajax({
        ...
        type: "get",
        url: contextPath+'/appversion/deleteImgsByNewsuuid.action',
        data:data,
        dataType: "json",
...
function upIdHaveFujian(upId)
{
	...
	$.ajax({
        ...
        dataType: "json",
        success: function (returndata) {
        	if(0 != returndata.uploadfile.id )//若没有附件，则生成一个附件实体，且id=0
        		{
        			returnFlag = true;
        		}
...
//	var url = 'uploadApkFile.jsp?uploadId='+uploadId;
	var url = contextPath+'/menu/uploadApkFile.action?uploadId='+uploadId;
	$('#'+dialogId).dialog({
	    title: '上传应用安装包文件',
	    width: 500,
	    height: 300,
