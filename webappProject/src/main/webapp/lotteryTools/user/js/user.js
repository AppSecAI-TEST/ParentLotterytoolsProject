...
//		queryParams: params,
	...
	function updateAccount(id)
	{
			 ...
			 * 用户修改
			 */
			var url = contextPath + '/user/getDetailUser.action';
	...
	{
		var url = contextPath + '/user/deleteUserByIds.action';
...
//				striped:true,
	...
	function initSelectedRoleGrid(id){
		...
		$('#selectedRoleGrid').datagrid({
			singleSelect:false,
			rownumbers:false,
			url:contextPath + '/user/getUserRelaRoleList.action',//'datagrid_data1.json',
			method:'get',
			queryParams : {
				id : id
			},
			...
			columns:[[  //{"roleId":"ff808181516ab01e01516ab2bd7e0000","code":"SC_DL","name":"市场代理","parentRolename":"市场专员","parentRole":"ff808181514698fb015146a085460001"}
					...
					{ field: 'parentUid', title: '上级名称', width: 100, align: 'left', 
						editor: { 
							type: 'combobox', 
							options: { 
								url : contextPath + '/user/getRoleRelaUserList.action',
	...
	function submitSelRoles(){
		...
		$.ajax({
			async: false,   //设置为同步获取数据形式
	        type: "post",
	        url: contextPath+'/user/saveUserRleaRole.action',
	        data: {
