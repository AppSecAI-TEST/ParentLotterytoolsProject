...
<html>
    ...
    <head>
      <link rel="stylesheet" href="<%=request.getContextPath() %>/lotteryTools/js/jquery.uploadify-v2.1.0/uploadify.css" type="text/css"></link>
       ...
       <script type="text/javascript" src="<%=request.getContextPath() %>/lotteryTools/js/jquery.uploadify-v2.1.0/jquery.uploadify.v2.1.0.min.js?f=<%=System.currentTimeMillis()%>"></script>
       ...
       $(function(){
           ...
           var num = 0;//选择的文件个数
            $('#file').uploadify({
             ...
             'script'          : '../servlet/UploadApk',//后台处理程序的相对路径 。默认值：uploadify.php 
             ...
             'fileExt'        : '*.apk;',//设置可以选择的文件的类型，格式如：'*.doc;*.pdf;*.rar' 
             'fileDesc'       : 'Web Office Files (.apk)',//这个属性值必须设置fileExt属性后才有效，用来设置选择文件对话框中的提示文本，如设置fileDesc为“请选择rar doc pdf文件”
          ...
          // 'scriptData'     :{ 'pack':proId}, // 多个参数用逗号隔开 'name':$('#name').val(),'num':$('#num').val(),'ttl':$('#ttl').val() 
           'method'          :'GET',
             ...
             'onComplete'   : function(event, queueID, fileObj, response, data) {
                 ...
                 var array = new Array('apk');  //可以上传的文件类型   
                ...
                if (uploadFlag == false) {  
                    $("#status-message").text("只可以上传'apk'格式的应用安装包");
                }  
                if(uploadFlag)
                	{
                		      $.ajax (
								 	 {
					          					  ...
					          					  url: contextPath+"/appversion/saveFujian.action",
